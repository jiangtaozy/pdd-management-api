 ...
 * Created by jemo on 2022.5.21 16:05:19
 * Place Order List
 * 待下单订单
 */
...
func PlaceOrderList(w http.ResponseWriter, r *http.Request) {
  start := time.Now()
  db := database.DB
  rows, err := db.Query(`
    SELECT
    ...
    WHERE
      ...
      and itemOrder.orderStatusStr = '待发货'
      and (
        order1688.orderStatus = 0
        or
        order1688.orderStatus is null
      )
